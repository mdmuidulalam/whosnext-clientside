...
export class TreatrequestsComponent implements OnInit {
  ...
  onClickShowMore() {
    ...
    for(var i: number = startIndex ; i < this.allpages.contents.length && i < startIndex + this.allpages.numberOfContentPerPage ; i++) {
      ...
      this.treatRequests[i].deadline = Helper.convertDateInCurrentTimeZone(this.treatRequests[i].deadline);
  ...
  getTreatRequests() {
    ...
    this.treatsService.getTreatRequests(pagingViewModel).subscribe((responce: ResponseViewModelWithAdditionInformation) => {
      if(responce.success){
  ...
  onConfirmationClick(treatId: number) {
    this.treatsService.confirmTreat(treatId).subscribe((responce: ResponseViewModel) => {
      if(responce.success){
